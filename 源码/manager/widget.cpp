    ...
    ui(new Ui::Widget)
{
    ...
    //stackedwidget是可以进行多个页面切换的，每个页面都可以放自己的控件，然后实现相关的功能。

    QSqlQueryModel *factoryModel = new QSqlQueryModel(this);
    //QSqlQueryModel类为SQL的结果集提供了一个只读的数据模型
...
void Widget::on_sellNumSpinBox_valueChanged(int arg1)
{
    if(arg1 == 0){
        ...
        total=QString::number(sum).toInt();
...
void Widget::on_sellOkBtn_clicked()
{
    ...
    if(srpt.exec()==QDialog::Accepted&&srpt.flag)
    {//exec是打开读卡界面的函数，且阻塞现在的函数，flag是读卡状态
        srpt.flag==false;
...
void Widget::createMenuBar()
{
    ...
    manageMenu = menuBar()->addMenu(tr("菜单"));
